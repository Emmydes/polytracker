...
function EntryTimer({ pick, intraday, expired, compact = false }) {
  ...
  if (expired) {
    return <span className="text-gray-500 text-sm">Entry closed</span>;
  }
...
export default function SignalCard({
  ...
  const expired = pick.status === 'EXPIRED';
  const marketPct = getMarketPct(pick);
  const smartPct = getSmartMoneyPct(pick);
  const divergence = getDivergencePct(pick);
  const enterBefore = getEnterBeforePrice(pick);
  const breakdown = getConfidenceBreakdown(pick, totalTrackedWallets);
  const categoryLabel = pick.market_category ? getCategoryLabel(pick.market_category) : 'General';
  const cardId = pick.id ?? `${pick.market_id}-${pick.recommended_side}`;
  ...
  useEffect(() => {
    ...
    fetch(`/api/market/${pick.market_id}`)
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => {
        if (cancelled || !data?.positions) return;
        setWallets(
          data.positions
            .filter((p) => p.side === pick.recommended_side)
            .map((p) => ({
              address: p.address,
              win_rate: p.win_rate,
  ...
  }, [expanded, pick.market_id, pick.recommended_side]);
    ...
    >
      ...
      <div className="pl-4 pr-4 pt-4 pb-3">
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <span
            className={`shrink-0 px-2.5 py-0.5 rounded-full text-xs font-display font-semibold ${confidenceBadgeStyle(pick.confidence)}`}
          >
            {pick.confidence ?? '—'}
          </span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm mb-2">
          <span className="font-display font-semibold text-gray-200">
            {pick.recommended_side} · {marketPct}%
          </span>
            ...
            <dl className="space-y-2 text-sm">
              <div className="flex justify-between gap-4">
                ...
                <dd className="font-display text-gray-200 text-right">
                  {formatPriceDollar(pick.current_price)} {pick.recommended_side}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-gray-500">Smart money target</dt>
                <dd className="font-display text-gray-200 text-right">
                  {formatPriceDollar(pick.entry_price)} {pick.recommended_side}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
              ...
              <div className="flex justify-between gap-4">
                ...
                <dd className="font-display text-gray-200 text-right">{pick.strength ?? 'MODERATE'}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-gray-500">Signal created</dt>
                <dd className="text-gray-300 text-right">{formatRelativeTime(pick.created_at)}</dd>
              </div>
              <div className="flex justify-between gap-4">
                ...
                <dd className="text-right">
                  {expired ? (
                    ...
                    <EntryTimer pick={pick} intraday={intraday} expired={false} compact />
                  )}
                </dd>
              </div>
            </dl>
          ...
          <section>
            <ConfidenceBreakdown
              breakdown={{
                ...
                divergenceRaw: Math.abs(Math.round(pick.drift_pct ?? divergence)),
                overall: breakdown.overall,
                trackedWallets: `${pick.elite_trader_count ?? 0} of ${breakdown.tracked}`,
              }}
            />
          </section>
